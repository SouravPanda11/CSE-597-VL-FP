...
def parse_function(*metrics, directory="", args=None, end_signal=None):
    ...
    for subdir in subdirs:
        ...
        print(f"Looking for file: {fpath}")  # Debug statement
        if not osp.exists(fpath):
            print(f"Log file not found: {fpath}")  # Debug statement
            continue  # Skip if log.txt is missing
        ...
        if output:
            ...
            print(f"No metrics found in {fpath}")  # Debug statement

    if len(outputs) == 0:
        raise ValueError(f"Nothing found in {directory}. Check the logs and end_signal.")
